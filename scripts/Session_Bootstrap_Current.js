...
if (!event || typeof event !== 'object') {
  event = { chat_id: 0, error: 'trigger_event_lost' };
...
const remoteEnvelope = (payload.session && typeof payload.session === "object") ? payload.session : payload;
const remoteData = (remoteEnvelope && typeof remoteEnvelope === "object") ? remoteEnvelope : {};
...
const session = {
  session_id: String(existing.session_id ?? ('sess_' + String(event.user_id ?? event.chat_id ?? 'guest'))),
  customer_id: String(existing.customer_id ?? event.user_id ?? ''),
