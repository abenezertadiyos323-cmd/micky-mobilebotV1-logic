...
const req = https.request({
  ...
  res.on('end', () => {
    ...
    // There may be a pinData attached to the node that we should remove too?
    if (w.pinData) {
        delete w.pinData['Admin Handoff Notify?'];
        delete w.pinData['Admin Handoff Telegram Send'];
    }

    const payload = JSON.stringify({ name: w.name, nodes: w.nodes, connections: w.connections, settings: w.settings, staticData: w.staticData });
    ...
    const pushReq = https.request({
      ...
      pushRes.on('end', () => {
        ...
        console.log("ERROR BODY:", pushData);
      });
    });
