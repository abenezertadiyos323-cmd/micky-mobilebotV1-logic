...
const client_config =
  input.client_config && typeof input.client_config === "object"
    ? input.client_config
    : {};
const understanding_output =
  input.understanding_output && typeof input.understanding_output === "object"
    ? input.understanding_output
    : {};
const rules_output =
  input.rules_output && typeof input.rules_output === "object"
    ? input.rules_output
    : {};
const resolverInput =
  rules_output.resolver_input && typeof rules_output.resolver_input === "object"
...
const shownProducts = Array.isArray(
...
const currentInterest = normalizeProduct(
  ...
  shownProducts.length + 10,
);
const requestedName =
  typeof resolverInput.resolved_product_name === "string" &&
  resolverInput.resolved_product_name.trim()
...
if (resolverInput.resolved_reference?.id) {
  selectedProduct =
    shownProducts.find(
      (product) => product.id === resolverInput.resolved_reference.id,
    ) ?? null;
}
if (!selectedProduct && requestedName) {
  selectedProduct =
    shownProducts.find((product) => {
...
const products = selectedProduct
  ? [selectedProduct]
  : shownProducts.slice(0, 3);
const numericPrices = products
  .map((product) => product.price_etb)
...
if (rules_output.reply_mode === "clarify_reference") {
  ...
  exchange_context = {
    ...
    extracted_need: understanding_output.extracted_need ?? null,
  };
